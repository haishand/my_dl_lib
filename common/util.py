...
def preprocess(text):
    """Tokenize text and map words to integer ids."""
    text = text.lower()
    text = text.replace(".", " .")
    words = text.split(" ")
...
def eval_perplexity(model, corpus, batch_size=10, time_size=35):
    """Evaluate language model perplexity on a corpus."""
...
def eval_seq2seq(model, question, correct, id_to_char, verbos=False, is_reverse=False):
    """Evaluate one seq2seq sample and optionally print details."""
...
def analogy(a, b, c, word_to_id, id_to_word, word_matrix, top=5, answer=None):
    """Solve word analogy queries using vector arithmetic."""
    for word in (a, b, c):
        if word not in word_to_id:
